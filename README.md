...
**<https://techc-hack1.vercel.app>**

ひとつの言葉が、別の誰かの言葉を生んでいく場所。

1日に1回、言葉が届く。読んで何かを思い出したら、自分の言葉を書く。
その言葉は明日、別の誰かに届く。仕組みはこれだけで、
コメントも、いいねも、フォローも、通知もない。
...
最後に、他人の軌跡へ「この言葉から、ことづてが生まれました」と現れる。

---

## このサービスが集めるもの

集めているのは名言ではなく、言葉と、それによって動いた自分のほうである。
...
自分の中で考えすぎる人。人に気を使いすぎて、言いたいことを飲み込む人を想定している。
会議で違和感を持っても「まあいいか」で終わらせる、友達の悪口に否定も肯定もできず黙る、
SNS に書こうとして投稿する直前に消す——そういう場面を持っている人である。

その人に必要なのは、自分に宛てられた返信ではないと考えた。
自分に向けて言われると身構えるし、名前を出して表明した意見は引っ込めにくい。
必要なのは、自分と関係のないところで生きている他人の世界にふれることだった。

だからこのサービスでは、誰も自分に宛てて書けない構造にしている。
いいねも返信も作っていないのは、機能を削ったのではなく、
宛先が存在しないことのほうを価値として選んだためである。
...
本を作るアプリではなく、影響が循環する場所として設計している。
蓄積(軌跡)は循環の結果として生まれるもので、目的ではない。
...
芥川の主題は「危険思想」、こもれびの主題は「友達の悪口」で、共通する内容はない。
残っているのは「この言葉がなければ、この言葉は生まれなかった」という関係だけである。

これは偶然ではない。書く欄の問いを「感想」ではなく「読んで、自分の何を思い出しましたか」に
した時点で、答えは必ず書き手の側の出来事になる。内容が変質するのは設計の帰結であって、
事故ではない。

そのため、ここで扱っているのは思想の伝播ではなく影響の循環だと考えている。
リツイートは内容が変わらずに広がるが、ここでは内容が変わらないと成立しない。
転送ボタンを置いていないのではなく、転送が原理的に成り立たない。
...
この言葉は、こもれびさんが三日坊主さんの言葉に触れて書いたものです
...
```
この言葉から、ことづてが生まれました
  │ 波風を立てたくない、が自分にもある。会議で違和感を…… — 三日坊主さん
```

「受け取りました」は到達した数だが、こちらは自分の言葉が誰かの考えを動かした証拠にあたる。
返信ではない。書いた人はこちらに宛てておらず、こちらから返す手段もない。
返ってくるのは、動かしたという事実だけである。

思想家を「有名な人」ではなく「誰かの考えを変えた人」と定義するなら、
その条件を満たした瞬間がこの画面に出ていることになる。
...
1. 偉人とユーザーが、同じ `words` テーブルに同じ形式で並ぶ。
   「思想家とは有名人ではなく、誰かの考えを変えた人である」という考えを、
   データの形のほうに置いた。分けて持つと、その時点で両者は別の存在になる。
   抽選も1つのテーブルを引くだけで済み、比率は `CLASSIC_RATIO` で調整する。

2. `parent_word_id` が来歴を保持する。血統や継承ではなく、
   「この言葉がなければ、この言葉は生まれなかった」だけを記録している。
   ニーチェ → Aさん → Bさん → あなた、という連なりを再帰クエリ1回でたどれる
   ([`getWordWithLineage`](lib/queries.ts))。

3. `deliveries` が3つの役割を兼ねる。
   - `unique (user_id, word_id)` で、同じ言葉を同じ人に二度届けない
   - 「あなたの言葉を◯人が受け取りました」を数える
   - 循環が実際に回っている証跡になる

軌跡の1点は独立したテーブルを持たない。自分が書いた言葉と、その `parent_word_id` で
完全に表せるため、`words` から導出している。

---

## 収録した言葉について

偉人の言葉48件は、すべてパブリックドメインの原典から手作業で採取している。
生成AIに名言を作らせたものは一件も含まれていない。

「偉人の名言」は、インターネット上で誤帰属や捏造がとりわけ多いジャンルである。
出典を追えない引用を混ぜた時点で、プロダクト全体の信頼性が落ちる。そのため、
...
訳出にはAIの支援を受けたが、原文を必ず併記しているので照合できる。
存在しない引用を作らせることと、実在する原文を移すことは別のもので、前者は行っていない。

なお、アプリの実行時にAIは呼び出していない。配信も抽選も来歴の表示も、すべてSQLだけで動く。
...
7. **循環が見える状態をデモ用データとして再現可能にした。**
   ...
   [`scripts/demo.ts`](scripts/demo.ts) が12人・13件の言葉と配信履歴を投入し、`/demo` から循環をその場で1周させられる。
   系譜は3本ある(芥川龍之介→のらねこ→三日坊主→こもれび の4世代、三木清→すずめ→ことりん、太宰治→ゆず→たんぽぽ)。
   1本だけだと手で作った1例に見えるため、複数の連なりが同時に成立していることを見せている。
...
npm run db:demo   # デモ用データ(12人・3本の系譜 + 配信履歴)
```
...
| [`db/demo.sql`](db/demo.sql) | デモ用データ(12人・3本の系譜 + 配信履歴) |
...
3. ホームで名前を入れて「はじめる」
4. **「受け取る」** — 言葉が浮かび上がる
5. **来歴**が見える(芥川龍之介 → のらねこ → 三日坊主 → こもれび)
