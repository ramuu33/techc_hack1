...
| | 画面 | 内容 |
|---|---|---|
| ホーム | `/` | 今日のことづて(来歴つき)・書く・「あなたの言葉が◯人に届きました」 |
| 2番目 | `/book` | わたしの本(蓄積) |
| 3番目 | `/others` | 他の人の本を読む |
...
flowchart TB
    ...
    SEED["data/words.seed.json<br/>偉人の言葉 66件"] -.->|"npm run db:seed"| W
```
...
「人生本の1ページ」は独立したテーブルを持たない。**ページ = 自分が書いた言葉 + その `parent_word_id`** で
...
**偉人の言葉66件は、すべてパブリックドメインの原典から手作業で採取している。生成AIに名言を作らせたものは一件も含まれていない。**

「偉人の名言」は、インターネット上で誤帰属・捏造が最も蔓延しているジャンルである。
出典を追えない引用を混ぜた時点で、プロダクト全体の信頼性が損なわれる。そのため、

1. 原典の本文を実際に取得する
2. 出典URL(`source_url`)をデータに保持する — 全66件
3. 原典から訳出したものは原文(`original`)を併記する — 21件
...
| データ保存 | Postgres (Supabase) | 他人の本を読むために共有が必要。ローカル開発は素の Postgres で同じスキーマが動く |
...
```bash
npm run db:seed   # スキーマ適用 + 偉人の言葉66件(何度実行してもよい)
...
2. ホームで名前を入れて「はじめる」
3. **「受け取る」** — 言葉が浮かび上がる
4. ユーザーの言葉が届いた場合、**来歴**が見える(芥川龍之介 → はるか → けんた → みお)
5. その場で心が動いたことを書き、**「ことづてを託す」**
6. `/demo` の「自分の最新の言葉を、他の全員に届ける」で循環を1周させる
   - 通常は相手がアプリを開いた時に抽選で届く。発表の場でそれを待てないため、同じ配信処理をその場で起こす。作られるデータは通常の配信とまったく同じ
7. ホームに戻ると **「あなたの言葉が、今日◯人に届きました」**
8. `/others` で他の人の本を開く
